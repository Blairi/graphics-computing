...
int main()
{
	...
	*/
	...
	glm::mat4 cabina(1.0); // matriz para guardar la cabina
	glm::mat4 torso(1.0); // matriz para guardar torzo
	glm::mat4 cabeza(1.0); // matriz para guardar torzo
	...
	while (!mainWindow.getShouldClose())
	{
		...
		/*
		* SE EMPIEZA EL DIBUJO DE LA GRUA
		*/
		...
		*/
		...
		//cabina = model; // guardando transformacion de cabina
		//modelaux = model; // evitar heredar escalamiento
		...
		///*
		//* renderizado de eje con esfera pequenia
		//*/
		...
		//modelaux = model; // evitamos heredar el escalamiento de abajo
		...
		///*
		//* para construir la base y ruedas, usamos la referencia que hacemos al inicializar el dibujado
		//* asi nos podemos ahorrar algunas lineas de codigo para transladar la base abajo de la cabina
		//*/
		//model = cabina;
		...
		//* base
		//*/
		//model = glm::translate(model, glm::vec3(0.0f, -1.9f, 0.0f)); // heredamos la traslacion de la cabina
		//modelaux = model; // evitar heredar las escalas
		//model = glm::scale(model, glm::vec3(5.5f, 1.0f, 3.7f));
		...
		//*/
		//model = modelaux;
		//model = glm::translate(model, glm::vec3(1.7f, -0.5f, -2.3f));
		//model = glm::rotate(model, glm::radians(90.0f), glm::vec3(1.0f, 0.0f, 0.0f)); // girar cilindro
		//model = glm::scale(model, glm::vec3(1.1f, 1.0f, 1.1f));

		//modelaux = model; // evitar heredar rotacion
		...
		//*/
		//model = modelaux;
		//model = glm::translate(model, glm::vec3(-3.14f, 0.0f, 0.0f));
		//// giramos en Y para dar el efecto de rueda girando
		//modelaux = model; // evitar heredar rotacion
		...
		//*/
		//model = modelaux;
		//model = glm::translate(model, glm::vec3(0.0f, 4.6f, 0.0f));
		//// giramos en Y para dar el efecto de rueda girando
		//modelaux = model; // evitar heredar rotacion
		//model = glm::rotate(model, glm::radians(mainWindow.getarticulacion7()), glm::vec3(0.0f, 1.0f, 0.0f));//n
		//color = glm::vec3(0.5f, 0.5f, 0.5f);
		//glUniform3fv(uniformColor, 1, glm::value_ptr(color));
		//glUniformMatrix4fv(uniformModel, 1, GL_FALSE, glm::value_ptr(model));
		//meshList[2]->RenderMeshGeometry(); // cilindro
		...
		//// giramos en Y para dar el efecto de rueda girando
		//modelaux = model; // evitar heredar rotacion
		//model = glm::rotate(model, glm::radians(mainWindow.getarticulacion8()), glm::vec3(0.0f, 1.0f, 0.0f));//m
		//color = glm::vec3(0.5f, 0.5f, 0.5f);
		//glUniform3fv(uniformColor, 1, glm::value_ptr(color));
		//glUniformMatrix4fv(uniformModel, 1, GL_FALSE, glm::value_ptr(model));
		//meshList[2]->RenderMeshGeometry(); // cilindro
		...
		*/
		model = glm::translate(model, glm::vec3(0.0f, 0.0f, -4.0f));
		torso = model; // guardando transformacion de torso
		modelaux = model; // evitar heredar escalamiento
		...
		*/
		model = modelaux;
		model = glm::translate(model, glm::vec3(3.0f, -2.3f, -2.0f));
		...
		*/
		...
		modelaux = model; // evitar heredar escalamiento y rotamiento
		...
		*/
		// referencia a cabeza
		model = modelaux = cabeza;
		model = glm::scale(model, glm::vec3(0.5f, 0.5f, 0.5f));
		modelaux = model;
		model = glm::translate(model, glm::vec3(5.5f, 1.5f, 2.3f));
